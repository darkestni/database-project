...
public class ProjectOperator implements PhysicalOperator {
    ...
    public ArrayList<ColumnMeta> outputSchema() {
        ...
        for (TabCol  target :  outputSchema){

            for (ColumnMeta col :  childSchema){
                boolean  tableMatches =  target.getTableName()==null||  "*".equals(target.getTableName())||  target.getTableName().equalsIgnoreCase(col.tableName);
                boolean  colimMatches =  target.getColumnName().equalsIgnoreCase(col.name);


                if (tableMatches&&  colimMatches){
                    result.add(col);
                    break;
                }
            }
